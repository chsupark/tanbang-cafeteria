...
# Copyright (c) 2020 chsupark(a.k.a w3bn00b), return0927
...
class tCafeteria:
    ...
    def parseCafeteria(self, date=fetchDate(), return_all=False):
        ...
        try:
            url = "http://%s/sts_sci_md00_001.do?insttNm=&schulCode=%s&schulCrseScCode=%s" + \
                  "&schulKndScCode=0%s&ay=%s&mm=%s&searchButton="
            url = url % \
                  (
    ...
    def parseSchedule(self, date=fetchDate(), return_all=False):
        ...
        if return_all:
            ...
            return schedules[_YEAR][_MONTH][_DAY]
